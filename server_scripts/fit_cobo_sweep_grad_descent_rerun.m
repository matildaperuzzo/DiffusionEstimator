...
for i = 1:numel(layer_sets)
    ...
    file_to_load = get_recent_fit_file(generated_data_dir, 'cobo', extra_layers);
    fprintf('Rerunning grad_descent for cobo with layers %s\n', strjoin(layers, ', '));
    fprintf('Loading %s at level 4\n', file_to_load);
    fit_model('cobo', layers, false, file_to_load, 4);
end

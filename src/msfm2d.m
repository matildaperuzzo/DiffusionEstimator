...
function T = msf2d(F, source, W)
    T = 999*ones(size(F));
    accepted = zeros(size(F));
    considered = zeros(size(F));
    ...
    if nargin > 2
        if size(W) ~= size(F)
            error('Weight matrix W must be the same size as F.');
        end
    else
        W = ones(size(F));
    end

    for z = 1:size(source, 2)
        x = source(1, z);
        y = source(2, z);
        T(x, y) = 0; % Set distance at source points to zero
        accepted(x, y) = 1; % Mark source points as accepted

        for i = 1:length(neighbours)
            n = neighbours(:,i);
            considered(x+n(1),y+n(2)) = 1;
            if considered(x+n(1), y+n(2)) == 1
                s = h(i) * W(x+n(1),y+n(2)) * F(x+n(1),y+n(2));
                T_candidate = T(x,y) + s;
                if T_candidate < T(x+n(1), y+n(2))
                    T(x+n(1), y+n(2)) = T_candidate;
                    considered(x+n(1), y+n(2)) = 1; 
                end
            end
...
all_reached = false;
while all_reached ~= true
    if all(accepted(:))
        all_reached = true;
        continue
    end
    % find considered note with lowest
    considered_times = 999*ones(size(F));
    considered_times(considered==1) = T(considered==1);
    
    [minTime, idx] = min(considered_times(:));
    
    if minTime == 999
        all_reached = true; % If no more pixels can be reached, exit loop
    else
        [x, y] = ind2sub(size(T), idx); % Get coordinates of the pixel with minimum time
        accepted(x, y) = 1; % Mark this pixel as accepted
        considered(x, y) = 0; % Remove from considered
        % Update the neighbours of the accepted pixel
        for i = 1:length(neighbours)
            n = neighbours(:,i);
            nx = x + n(1);
            ny = y + n(2);
            
            if nx > 0 && nx <= size(F, 1) && ny > 0 && ny <= size(F, 2) % check that pixel is within bounds
                if (accepted(nx,ny) == 1)
                    continue
                end
                s = h(i) * W(nx, ny) * F(nx, ny);
                accepted_times = 999*ones(size(T));
                accepted_times(accepted == 1) = T(accepted == 1);
                T_candidate = calculate_T(accepted_times,nx,ny,s);
                considered(nx, ny) = 1; % Mark as considered
                
                if T_candidate < T(nx, ny)
                    T(nx, ny) = T_candidate; % Update the time 
                end
            end
        end
    end
...
function T_candidate = calculate_T(T,i,j,s)
    [nx, ny] = size(T);
    if i == 1
        a = T(i+1,j);
    elseif i == nx
    ...
    T_candidate = calculate_T_ab(a,b,s);

end

function T_candidate = calculate_T_ab(a, b, s)
    if abs(a - b) >= s
        T_candidate = min(a,b) + s;
    else
        T_candidate = 0.5 * (a + b + sqrt(max(0, 2*s^2 - (a-b)^2)));
    end
end
